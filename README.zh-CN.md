...
本仓库发布 **`tunan`** 插件，包含 43 个代理、66 个技能和 4 个 MCP 服务器——以原生插件形式支持 Claude Code、Codex 和 OpenCode，以 npx 形式支持 Reasonix。
...
### 原生插件（推荐用于 Claude Code、Codex、OpenCode）

如需更深入的集成（斜杠命令、MCP 自动加载），可通过各工具的 CLI 安装为原生插件。安装后重启 agent。
...
Reasonix 可以从本地克隆直接安装——脚本会将 `skills/` 复制到 Reasonix 目录：
...
使用 `--force`（Windows 上为 `-Force`）可覆盖已有技能。

> Claude Code、Codex、OpenCode 请直接使用上面的原生插件命令安装，无需 clone。install 脚本虽然也接受 `--claude` / `--codex` / `--opencode` 参数（内部委托给各平台的 plugin CLI），但同样是从 marketplace 安装，不会用到本地 checkout 源码。
