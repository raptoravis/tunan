...
| `status`                                                        | Read-only snapshot of what's left — lists open PRs and open issues. Defaults to current user; `--user <name>` queries a specific user, `--all` shows everyone, `--req` filters issues to `yunxing:req`. Creates nothing (unlike `retro`)                                                    |
