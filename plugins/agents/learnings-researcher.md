...
**Why this works:** `gh issue list --search` returns only matching feature issues in one JSON payload, so the candidate set (typically 5-20 issues instead of the whole label) is narrowed without N separate reads; the solution comment for each survivor is fetched in Step 3.
...
### Step 3: Read Each Candidate's Solution Comment and Parse Its Frontmatter Block
...
Only for issues that pass the filter (strong or moderate matches), read the complete solution comment — the markdown sections below the frontmatter block — to extract:
...
- Parse only the ```yaml frontmatter block of each candidate's solution comment for filtering
- Fully read only the solution comments of candidates that pass relevance scoring in Step 5
