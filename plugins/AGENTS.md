...
**This plugin's `AGENTS.md` and `CLAUDE.md` files are authoring context — they do not ship with the installed plugin.** Skills are packaged and installed into end-user environments (their own repos, or folders that may not even be git repos), where they run against _the user's_ `AGENTS.md`/`CLAUDE.md`, not this repo's.
...
- [ ] **Put the rule INLINE in `SKILL.md`, not only in `references/`.** Because the menu fires deep in a phase, the blocking-tool directive must live in the SKILL.md phase where the question fires (or in a top-of-skill `## Interaction Method` section), not solely in a reference that may not be loaded by the time the menu renders. A skill whose only blocking-tool guidance sits in `references/` reliably regresses to ad-hoc chat menus. Routing through the `align` skill is equivalent only when the SKILL.md explicitly instructs loading it at the decision point.
- [ ] **Option overflow still routes through the blocking tool — count alone never justifies a chat list.** `AskUserQuestion` caps at 4 options, and this is a hard platform limit: a 5th option is rejected by the tool schema, not merely discouraged. When a menu has more than 4 genuinely relevant destinations, do **not** render a numbered list to fit them all. Fire the blocking tool with the 4 most important options and name the remaining destinations in the question stem, relying on the tool's built-in free-form / "Other" input so they stay reachable (e.g., stem: "…or describe another path, such as opening it in Proof"). Prefer first to cut, merge, or demote-to-prose any option that doesn't earn a slot — but even when 5+ destinations are all genuinely distinct, the answer is "4 in the tool + the rest in the stem," never a numbered list. The numbered-list fallback is reserved for harnesses with no blocking tool or a tool-call error; option count is never a reason to use it.
...
- [ ] Keep total options to 4 or fewer (`AskUserQuestion` caps at 4 across platforms we target — a 5th option is rejected by the schema). When more than 4 destinations are relevant, present the 4 most important as options and surface the rest in the question stem for free-form selection rather than overflowing to a chat list
...
- [ ] Phrase labels from the user's intent, not the system's internal state — each option should complete "I want to \_\_\_" from the user's POV; avoid leaking mode names like "end-sync" or "phase-3" into labels
...
- [ ] **Never interpolate a space- or quote-bearing value into a native-command argument.** Windows PowerShell 5.1 mangles the escaping when a value containing spaces, embedded quotes, or `<>!` is interpolated into an argument passed to an external exe via `& exe ...` — the classic trap is building a `gh ... --jq ".[] | select(.body | startswith(\"$marker\"))"` filter where `$marker` is `<!-- tunan:solution -->`: the jq expression arrives corrupted and silently matches nothing (a false negative, not an error). Keep the native arg a fixed, space-free literal and move the variable comparison into PowerShell: fetch raw JSON (`gh api <url>`, the only native arg being the URL), `ConvertFrom-Json`, and compare with `.StartsWith($marker, [System.StringComparison]::Ordinal)`. When piping non-ASCII through a native command, set `[Console]::OutputEncoding = [System.Text.Encoding]::UTF8` first (and restore it) so `ConvertFrom-Json` doesn't choke on UTF-8 under the legacy code page. The bash twin has no such bug (it passes single args correctly), so this fix is PowerShell-only and does not change the shared stdout/arg contract. See `lfg/scripts/gate.ps1` `Has-MarkerComment`.
...
- [ ] **Pre-resolution exception:** `!` backtick pre-resolution commands run at skill load time, not at agent runtime. They may use chaining (`&&`, `||`), error suppression (`2>/dev/null`), and fallback sentinels (e.g., `|| echo '__NO_CONFIG__'`) to produce a clean, parseable value for the model. This is the preferred pattern for environment probes (CLI availability, config file reads) that would otherwise require runtime shell calls with chaining. Three shapes are rejected by Claude Code's safety check and must be avoided in `!` backticks:
  ...
  **Permission gate on extracted scripts — invoke from the skill body, not from `!` pre-resolution.** A pre-resolution `bash "${CLAUDE_SKILL_DIR}/scripts/<name>.sh"` form passes the safety check but trips Claude Code's permission check at skill-load time, which does _not_ honor `defaultMode: bypassPermissions`. Allow-listing via `allowed-tools` frontmatter is unreliable at _load time_: empirically, broad `Bash(bash *)` patterns appear to load with bypass on but narrow filename-pinned patterns like `Bash(bash *upstream-version.sh)` fail with bypass off. Move the script invocation into the skill body so it runs via the runtime Bash tool instead. Two pieces are required for it to actually work:
