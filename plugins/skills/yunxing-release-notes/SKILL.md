---
name: yunxing-release-notes
description: Summarize recent yunxing plugin releases, or answer a specific question about a past release with a version citation. Use when the user types `/yunxing-release-notes` or asks "what changed in yunxing recently?" or "what happened to `<skill-name>`?".
...
Look up what shipped in recent releases of the yunxing plugin. Bare invocation summarizes the last 5 plugin releases. Argument invocation searches the last 40 releases and answers a specific question, citing the release version that introduced the change.

Data comes from the GitHub Releases API for `EveryInc/yunxing-plugin`, filtered to the `yunxing-v*` tag prefix so sibling components (`cli-v*`, `coding-tutor-v*`, `marketplace-v*`, `cursor-marketplace-v*`) are excluded.
...
Version-like inputs (`2.65.0`, `v2.65.0`, `yunxing-v2.65.0`) are query strings, not a separate lookup-by-version mode. They flow through query mode like any other text.
...
> `python3` is required to run `/yunxing-release-notes`. Install Python 3.x and retry, or open https://github.com/EveryInc/yunxing-plugin/releases directly.
...
```json
{
  ...
  "fetched_at": "...",
  "releases": [
    {"tag": "yunxing-v2.67.0", "version": "2.67.0", "name": "...",
...
If `ok: true`, take the first 5 entries from `releases` (the helper has already filtered to `yunxing-v*` and sorted newest first). If fewer than 5 are available, render whatever count came back without warning.
...
```
Showing the last 5 releases. For older history, ask a specific question (e.g., `/yunxing-release-notes what happened to <skill>?`).
Browse all releases at https://github.com/EveryInc/yunxing-plugin/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo EveryInc/yunxing-plugin --json title,body,url
```
...
```
I couldn't find this in the last 40 plugin releases. Browse the full history at https://github.com/EveryInc/yunxing-plugin/releases
```
