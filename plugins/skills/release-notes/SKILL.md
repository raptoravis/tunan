...
Data comes from the GitHub Releases API for `raptoravis/yunxing-plugin`, filtered to the `yunxing-v*` tag prefix so sibling components (`cli-v*`, `coding-tutor-v*`, `marketplace-v*`, `cursor-marketplace-v*`) are excluded.
...
> `python3` is required to run `/yunxing:release-notes`. Install Python 3.x and retry, or open https://github.com/raptoravis/yunxing-plugin/releases directly.
...
Browse all releases at https://github.com/raptoravis/yunxing-plugin/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo raptoravis/yunxing-plugin --json title,body,url
```
...
```
I couldn't find this in the last 40 plugin releases. Browse the full history at https://github.com/raptoravis/yunxing-plugin/releases
```
