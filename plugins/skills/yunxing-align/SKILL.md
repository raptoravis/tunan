...
# yunxing-align — AI-initiated alignment with pre-selected defaults
...
- **Pre-select the best**: place it first and append `(Recommended)` to its label. This is what lets the sponsor take the default and land on the optimal choice.
...
> **Q1 — Where should the rate limiter live?**
>
> - `Edge middleware (Recommended)` — one choke point, no per-route changes; adds a small hop
> - `Per-service interceptor` — finer control; must be wired into each service
> - `API gateway config` — zero code; limited to the gateway's coarse buckets
>
> **Q2 — Limit window granularity?**
>
> - `Per-minute (Recommended)` — matches typical abuse patterns; simple to reason about
> - `Per-second` — stops bursts, heavier bookkeeping
> - `Per-hour` — too coarse for abuse, fine for quotas
