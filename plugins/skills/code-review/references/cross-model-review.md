...
```bash
SKILL_DIR="<absolute path of the directory containing the code-review SKILL.md you read>";
CROSS_MODEL_HOST_HARNESS="<host-harness>" CROSS_MODEL_FIXED_ROUTE="<fixed-route>" python3 "$SKILL_DIR/scripts/peer-job-runner.py" start --skill code-review --run-id "<run-id>" --label adversarial -- env CROSS_MODEL_HOST_HARNESS="<host-harness>" CROSS_MODEL_FIXED_ROUTE="<fixed-route>" bash "$SKILL_DIR/scripts/cross-model-adversarial-review.sh" "<host-serving-family>" "<target>" "<base-ref>" "<run-dir>"
```

**Native Windows (no Git Bash):** `peer-job-runner.py` and the orchestrator both run on Windows Python; substitute the PowerShell twin for the bash worker — `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$SKILL_DIR/scripts/cross-model-adversarial-review.ps1"` in place of `bash "$SKILL_DIR/scripts/cross-model-adversarial-review.sh"` (identical positional args and JSON contract), and `python`/`py -3` for `python3`. On macOS/Linux (or Windows via Git Bash) use the bash form above unchanged.

- `<run-id>` = the Stage 4 run id (the same one that forms `<run-dir>`); job state lives under `/tmp/tunan/code-review/<run-id>/jobs/<job-id>/`.
...
- `<run-dir>` = the Stage 4 run dir (`${TMPDIR:-/tmp}/tunan/tunan:code-review/<run-id>/`). The script writes `adversarial-<provider>.json` there **only after** forcing `reviewer` to `adversarial-<provider>` and downgrading peer `safe_auto` → `gated_auto`.
...
- After fold-in (or after deadline reaping), delete the consumed job directory (`/tmp/tunan/code-review/<run-id>/jobs/<job-id>/`) — its log and result are review content and must not outlive their use.
...
**Isolation differs from doc-review by design.** Doc-review embeds a self-contained document into a tool-less empty scratch. Code-review needs surrounding code context, so peers run **in-tree read-only**:
