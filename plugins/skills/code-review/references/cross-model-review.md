...
Invoke it via the skill-dir anchor — set `SKILL_DIR` to the absolute directory of **this** skill's `SKILL.md` (the one you read to run code-review), because the Bash tool's CWD is the user's project, not the skill dir, on every host:
...
- `<run-dir>` = the Stage 4 run dir (`${TMPDIR:-/tmp}/tunan/tunan:code-review/<run-id>/`). The script writes `adversarial-<peer>.json` there.
