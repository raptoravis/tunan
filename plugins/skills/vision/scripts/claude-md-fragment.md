...
3. 每张截图用 `tunan:vision` skill 分析：
   ...
   ```
   可用 `--provider qwen|openai|siliconflow|doubao` 切换模型；未指定时按 `VISION_PROVIDER` / 首个 key 自动选。
4. 汇总所有截图的分析结果，列出完整、去重的问题清单后再报告。

key 与 provider 配置见 `~/.env`（`SILICONFLOW_API_KEY` / `DASHSCOPE_API_KEY` / `OPENAI_API_KEY` / `DOUBAO_API_KEY` 等），`vision.py` 每次运行自动加载。
