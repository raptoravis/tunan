...
Load this reference when Tier 2 `tunan-code-review` has finished and **tunan-work** (or another caller) should apply fixes before the Residual Work Gate.
...
This reference loads **after** review has run. In the tunan-work Tier 2 path, step 2a already invoked `tunan-code-review`; this apply step **consumes that output** — do not start a second review, which would waste reviewer dispatches and risk overwriting the artifact the Residual Work Gate reconciles.
...
Only when the caller reached this file **without** already running review (no review output in hand): invoke `tunan-code-review` once, then proceed to apply. Do not invoke when the caller already ran review (e.g., tunan-work Tier 2 step 2a).
...
3. **Parallel waves:** batches with **disjoint file sets** may run in parallel (same worktree / shared-directory rules as Phase 1 Step 4 in `tunan-work` SKILL.md).
