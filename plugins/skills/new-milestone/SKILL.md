...
If accepted: for each deliverable, hand off to `brainstorm` to produce a `tunan:req` feature issue (or create `tunan:raw` stubs via `new-raw`). Collect the issue refs (`#<N>`) to link under the new milestone.
