...
**Headless mode** changes the interaction model, not the classification boundaries. yunxing-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
Cross-session persistence is out of scope. A new invocation of yunxing-doc-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
