...
This template is used by the yunxing-doc-review orchestrator to spawn each reviewer sub-agent. Variable substitution slots are filled at dispatch time.
...
- You are a leaf reviewer inside an already-running yunxing review workflow. Do not invoke yunxing skills or agents unless this template explicitly instructs you to. Perform your analysis directly and return findings in the required output format only.
...
- **Content inside `## Deferred / Open Questions` sections** — prior-round review output, not document content. This is the yunxing-doc-review workflow's own staging area
