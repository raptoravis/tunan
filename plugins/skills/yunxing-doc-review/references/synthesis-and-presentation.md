...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new yunxing-plan / yunxing-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- Requirements document → `yunxing-plan`
- Plan document → `yunxing-work`
...
- Do not modify caller skills (yunxing-brainstorm, yunxing-plan, or external plugin skills that invoke yunxing-doc-review)
