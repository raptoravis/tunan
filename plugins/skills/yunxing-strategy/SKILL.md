...
`yunxing-strategy` produces and maintains `STRATEGY.md` - a short, durable anchor document that captures what the product is, who it serves, how it succeeds, and where the team is investing. It lives at the repo root as a canonical, well-known file (peer of `README.md`). Downstream skills (`yunxing-ideate`, `yunxing-brainstorm`, `yunxing-plan`) read it as grounding when it exists.
...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `yunxing-brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that `yunxing-ideate`, `yunxing-brainstorm`, and `yunxing-plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `yunxing-ideate` or `yunxing-brainstorm` skills as a next step.
...
- Does not write product requirements or implementation plans - those are `yunxing-brainstorm` and `yunxing-plan`.
