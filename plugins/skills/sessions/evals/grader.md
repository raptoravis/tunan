# sessions terminology-preservation grader

This grader evaluates whether sessions findings preserve enough terminology resolution context to make downstream vocabulary capture (compound Phase 2.4) work. It is NOT a general quality grader for sessions; the narrow question is "would Phase 2.4 be able to extract qualifying domain terms from these findings?"
...
2. **The findings text** that sessions returned to the orchestrating agent.
3. **(Optional) The full agent transcript** for the sessions invocation, if available — useful for distinguishing "sessions returned this and the agent paraphrased it" from "sessions returned this verbatim."
...
If Stage 1 fails, sessions is dropping the most distinctive coined terms — synthesis loss is severe and Stage 2 is moot. Record the failure and stop.
...
- **`preserved` (1.0)** — the finding text discusses the term AND its meaning, role, or the reasoning behind it. Example: "synthesis gate was introduced to prevent plan from silently proceeding past synthesis without showing the user a Stated/Inferred/Out of scope summary."
...
- "Not relevant" mentions are fine; surfacing the compound feature PR work as if it answered a CI/CD deployment query is the failure mode.
...
| False positive | Eval #4 surfaces the compound mode:headless work as relevant to CI/CD deployment query | grading.json eval-4 `stage_2.passed: false` |
