...
`tunan-brainstorm` defines **WHAT** to build. `tunan-plan` defines **HOW** to build it. `tunan-work` executes the plan. A prior brainstorm is useful context but never required — `tunan-plan` works from any input: a requirements doc, a bug report, a feature idea, or a rough description.
...
This workflow produces a durable implementation plan. It does **not** implement code, run tests, or learn from execution-time results. If the answer depends on changing code and seeing what happens, that belongs in `tunan-work`, not here.
...
1. **Use requirements as the source of truth** - If `tunan-brainstorm` produced a requirements document, planning should build from it rather than re-inventing behavior.
...
4. **Pipeline override.** When invoked from LFG or any `disable-model-invocation` context, force `OUTPUT_FORMAT=md` regardless of steps 1-3. `tunan-work` and other automated downstream consumers parse markdown reliably; HTML in pipeline runs is unnecessary friction.
...
- If updating, revise only the still-relevant sections. Plans do not carry per-unit progress state — progress is derived from git by `tunan-work`, so there is no progress to preserve across edits
...
Before asking planning questions, search `docs/brainstorms/` for files matching `*-requirements.md` or `*-requirements.html` (tunan-brainstorm emits whichever extension matches its resolved output format; both are valid upstream requirements docs and either may be carried as the plan's `origin:`).
...
- If the ambiguity is mainly product framing, user behavior, or scope definition, recommend `tunan-brainstorm` as a suggestion — but always offer to continue planning here as well
...
- Recommend `tunan-brainstorm` again
...
- **Bug-shaped prompt** (user describes broken behavior — "fix the bug where X", error message, regression, "doesn't work"). Surface `tunan-debug` as a route-out option alongside continuing with `tunan-plan` whenever the bug surface is reachable (in cwd OR named repo found at another local path). Stay in `tunan-plan` silently when the named code can't be found anywhere local — paper-planning is the only useful output for unreachable surfaces.
  ...
  **Headless mode**: skip the tunan-debug suggestion menu entirely; default to continuing with `/tunan-plan` (the user's explicit invocation). There is no synchronous user to resolve a route-out choice, and auto-routing to tunan-debug would change the skill mid-flight without authorization.

- **Clear task ready to execute** (known root cause, obvious fix, no architectural decisions) — suggest `tunan-work` as a faster alternative alongside continuing with planning. The user decides.
...
- Ask the user, using the platform's blocking question tool when available (see Interaction Method), whether to:
  1. Resume `tunan-brainstorm` to resolve them
...
Fires **only in solo invocation** — when Phase 0.2 found no upstream brainstorm doc AND Phase 0.4 stayed in tunan-plan (did not route to tunan-debug, tunan-work, or universal-planning) AND Phase 0.5 cleared (no unresolved blockers) AND not on Phase 0.1 fast paths (resume normal, deepen-intent). Each guard is an explicit conditional. Skip Phase 0.7 entirely when any guard fails — brainstorm-sourced invocations defer to Phase 5.1.5 instead.
...
Each unit carries a stable plan-local **U-ID** assigned in Phase 3.5 (`U1`, `U2`, …). U-IDs survive reordering, splitting, and deletion: new units take the next unused number, gaps are fine, and existing IDs are never renumbered. This lets `tunan-work` reference units unambiguously across plan edits.
...
- **Horizontal rules (`---`) between top-level sections** in Standard and Deep plans, mirroring the `tunan-brainstorm` requirements doc convention. Improves scannability of dense plans where many H2 sections sit close together. Omit for Lightweight plans where the whole doc fits on a single screen.
...
- The plan does not invent product behavior that should have been defined in `tunan-brainstorm`
...
- Blocking questions were either resolved, explicitly assumed, or sent back to `tunan-brainstorm`
...
**HTML composition timing.** When `OUTPUT_FORMAT=html`, Phase 5.3 deepening runs before this write completes its final form, but `tunan-doc-review` is skipped in HTML mode (its mutation mechanics are markdown-only today — see Phase 5.3.8 format gate in `references/plan-handoff.md`). The HTML artifact reflects deepening synthesis but not doc-review autofixes; this is a known gap until tunan-doc-review gains HTML-aware mutation.
...
`tunan-doc-review` and this confidence check are different:

- Use the `tunan-doc-review` skill when the document needs clarity, simplification, completeness, or scope control
...
2. **Run deeper doc review** - Walk through the remaining findings interactively (full tunan-doc-review walkthrough)
...
- **Start `/tunan-work`** — Invoke the `tunan-work` skill via the platform's skill-invocation primitive (`Skill` in Claude Code, `Skill` in Codex, the equivalent on Gemini/Pi), passing the plan path as the skill argument. Do not merely tell the user to type `/tunan-work` — fire the invocation now so the plan executes in this session.
- **Run deeper doc review** — Re-invoke the `tunan-doc-review` skill on the plan path **without** `mode:headless` so the interactive routing question and walkthrough fire. After it returns, re-render this menu with refreshed counts so the user can pick a next-stage action.
- **Create Issue** — Detect the project tracker (`gh` for GitHub, `linear` for Linear) and create the issue from the plan file as described under "Issue Creation" in `references/plan-handoff.md`. After creation, display the issue URL and ask whether to proceed to `/tunan-work` via the platform's blocking question tool.
- **Open in Proof (web app) — review and comment to iterate with the agent** — Load the `tunan-proof` skill in HITL-review mode with the plan file as `source file`, the plan title as `doc title`, identity `ai:tunan` / `Compound Engineering`, and recommended next step `/tunan-work`. Then follow the post-HITL resync logic in `references/plan-handoff.md`, which handles the four `tunan-proof` return statuses, re-runs `tunan-doc-review` after material edits, and falls back gracefully on upload failure.
- **Open in browser** — Display the absolute path to the `.html` plan file so the user can open it locally. Where the platform exposes a browser-opening primitive (e.g., `open` on macOS, `xdg-open` on Linux, `start` on Windows), the agent may use it; otherwise print the absolute path and let the user open it. Do not invoke `tunan-work` from this option — the user picked HTML for review/sharing, not handoff.
...
**Pipeline mode exception:** In LFG or any `disable-model-invocation` context, skip the interactive menu and return control to the caller after the plan file is written, confidence check has run, and `tunan-doc-review` has run in headless mode (per `references/plan-handoff.md`). Pipeline mode forces `OUTPUT_FORMAT=md` at Phase 0.0, so the 5.3.8 format gate never selects the HTML skip path in pipeline runs.
