...
This content is loaded when a synthesis-summary phase fires in tunan-plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in tunan-plan (did not route to tunan-debug, tunan-work, or universal-planning)
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (tunan-doc-review, tunan-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (tunan-doc-review, tunan-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/tunan-brainstorm`), "this is just a fix, no plan needed" (suggest `/tunan-work`), or "I need to investigate first" (suggest `/tunan-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/tunan-brainstorm` to revise the upstream doc).

In either case: stop tunan-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
