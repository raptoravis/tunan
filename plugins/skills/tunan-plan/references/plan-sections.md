...
- **The implementing agent** (`tunan-work` or a human) starts from an informed
...
Not every invocation of `tunan-plan` should produce a plan document. For
genuinely atomic work, the doc is ceremony — the implementer (whether
`tunan-work` or a human) can act directly without IDed units, KTDs, or
...
When skipping the plan doc, the work proceeds directly to `tunan-work` or to
...
- **Implementation Units** (with stable U-IDs) — the discrete units of work,
  sized so each is independently landable. `tunan-work` consumes these to
...
- **`status`** — `active` on creation; `tunan-work` flips to `completed` on
  ship. `tunan-plan`'s Phase 0.1 resume fast path keys on `active`. In HTML,
...
- **`origin`** — repo-relative path to an upstream brainstorm requirements
  ...
  and re-resolved when `tunan-plan` re-deepens. The HITL Proof flow uses
