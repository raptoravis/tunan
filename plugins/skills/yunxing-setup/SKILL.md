...
Interactive setup for yunxing — diagnoses environment health, cleans obsolete repo-local CE config, and helps configure required tools. Review agent selection is handled automatically by `yunxing-code-review`; project-specific review guidance belongs in `CLAUDE.md` or `AGENTS.md`.
