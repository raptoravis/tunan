...
`tunan-ideate` precedes `tunan-brainstorm`.

- `tunan-ideate` answers: "What are the strongest ideas worth exploring?"
- `tunan-brainstorm` answers: "What exactly should one chosen idea mean?"
- `tunan-plan` answers: "How should it be built?"
...
3. **Route action into brainstorming** - Ideation identifies promising directions; `tunan-brainstorm` defines the selected one precisely enough for planning. Do not skip to planning from ideation output.
...
- Never ask about solution direction, constraints, audience, tone, success criteria, or anything that characterizes the subject — those belong to `tunan-brainstorm`.
...
- Stop as soon as the subject is identifiable or the user has delegated to "Surprise me." More than 3 total questions across 0.2 and 0.4 is a smell that ideation is not the right workflow — consider suggesting `tunan-brainstorm`.
...
2. **Learnings search** _(elsewhere-software only; skipped by default in elsewhere-non-software)_ — dispatch `tunan-learnings-researcher` with the topic summary in case relevant institutional knowledge exists (skill-design patterns, prior solutions in similar shape). Skip for elsewhere-non-software: the CWD's `docs/solutions/` is unlikely to be topically relevant for non-digital topics, and running it risks polluting generation with unrelated engineering patterns.
