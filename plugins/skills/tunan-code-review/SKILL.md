...
- `docs/brainstorms/*` -- requirements documents created by tunan-brainstorm
- `docs/plans/*.md` -- plan files created by tunan-plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
7. **Learnings & Past Solutions.** Surface tunan-learnings-researcher results: if past solutions are relevant, flag them as "Known Pattern" with links to docs/solutions/ files.
...
After Stage 6, stop. Never push, open PRs, or file tickets from this skill. In default (interactive) mode, Stage 5c has already applied and (on a clean pre-review tree) committed the safe fixes; in `mode:agent` the review mutates nothing — the caller (for example `tunan-work`) and the user apply fixes, file tickets, or accept residual risk using the report and artifact.
