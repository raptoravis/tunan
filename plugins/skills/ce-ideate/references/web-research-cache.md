...
Files live under `<scratch-dir>/web-research-cache.json`, where `<scratch-dir>` is `${TMPDIR:-/tmp}/tunan/ce-ideate/<run-id>`, resolved once in SKILL.md Phase 1.
...
```bash
SCRATCH_ROOT="${TMPDIR:-/tmp}/tunan/ce-ideate"
