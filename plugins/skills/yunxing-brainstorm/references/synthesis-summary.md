...
- **Naming implementation detail in any bullet**: file paths, module names, exact JSON keys, HTTP status codes, error message wording, SQL syntax. The synthesis is scope-only; implementation is yunxing-plan's job. These granularity rules apply to every bullet in every section.
...
- "Use Postgres for persistence" — implementation detail (yunxing-plan's job), failed granularity rules
