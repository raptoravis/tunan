...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `tunan-brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that `tunan-ideate`, `tunan-brainstorm`, and `tunan-plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `tunan-ideate` or `tunan-brainstorm` skills as a next step.
...
- Does not write product requirements or implementation plans - those are `tunan-brainstorm` and `tunan-plan`.
