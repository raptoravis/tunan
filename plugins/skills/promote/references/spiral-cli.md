...
Spiral (`@every-env/spiral-cli`) drafts copy in a user's brand voice. `promote` uses it as an **optional enhancement** — every call must be wrapped so a missing, unauthed, or erroring CLI never blocks the skill.
...
This one-call cross-channel set is the ideal fit for `promote` when the user wants to announce across surfaces.
