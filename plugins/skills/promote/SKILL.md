...
Read `references/spiral-cli.md` for the exact setup prompt, the connect/install steps, and how the opt-out is recorded so later runs skip this. Present the setup offer with the platform's blocking question tool: `AskUserQuestion` in Claude Code (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded), `request_user_input` in Codex, `ask_user` in Gemini, `ask_user` in Pi — only fall back to a numbered list of the two options in chat when no blocking tool exists or the call errors (see below). In short:
