...
If it is missing, tell the user: "`agent-browser` is not installed. Use the `setup` skill to print the current install command, then install `agent-browser` and retry." Then stop. An installed discovery skill does not imply that the CLI or its browser runtime is installed.
