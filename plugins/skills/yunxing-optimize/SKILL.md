...
1. **Run `/yunxing-code-review`** on the cumulative diff (baseline to final). Load the `yunxing-code-review` skill on the optimization branch (interactive or `mode:agent`). To land eligible fixes before the next option, apply the mechanical-apply bar below.
