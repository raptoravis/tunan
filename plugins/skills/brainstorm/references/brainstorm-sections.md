...
- **Key Decisions** — include when the brainstorm produced opinionated
  ...
  unexamined proposal. `plan` enrichment inherits these labels into
  plan KTDs.
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("doc-review can act on this
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to plan, no italic provenance lines. Engineering process
