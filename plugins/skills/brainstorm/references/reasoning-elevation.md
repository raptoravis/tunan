...
Before reading any Fable config key, parsing Fable intent, or emitting any Fable string, self-identify the host with the same env-var union `code-review` uses:
...
2. **Config** — otherwise the per-skill key: `plan_use_fable` for plan, `brainstorm_use_fable` for brainstorm. Read it from the config file the **same way this skill's Phase 0.0 already resolves `plan_output` / `brainstorm_output`**: reuse the repo root the skill already resolved if you have it, else run `git rev-parse --show-toplevel`, then read `<repo-root>/.tunan/config.local.yaml` with the native file-read tool. This skill already read that file once at Phase 0.0 — reuse that result if you still have it rather than re-reading. Ignore commented (`#`-prefixed) lines. `true` → elevate; missing / commented / invalid / `false` / no file → off.
...
The elevated steps: **plan** — interpret research findings and author the plan, folded into one interpret-then-author call. **brainstorm** — generate approaches. The brainstorm integration-check consult is deferred and is NOT wired in this version.
...
"Shown once" is enforced by a **per-user marker file at a stable path outside the repo** (per-user, not per-checkout — e.g. `~/.config/tunan/fable-nudge-seen`). Before showing: if the marker exists, skip the nudge. After showing: create the marker (with its parent dir). A missing marker means "not yet shown."

- **plan:** `💡 Tip: add "use fable" to your prompt and Fable will author your plan with deeper reasoning — your session model stays as-is. Set plan_use_fable: true to make it the default.`
- **brainstorm:** `💡 Tip: say "use fable" and Fable will generate sharper approaches — no session switch needed. Set brainstorm_use_fable: true to default it on.`
