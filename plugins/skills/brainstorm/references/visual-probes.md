...
- Start: `node <resolved-helper-path> start --root ${TMPDIR:-/tmp}/tunan/brainstorm-visual/<run-id>`
- Start foreground: `node <resolved-helper-path> start --root ${TMPDIR:-/tmp}/tunan/brainstorm-visual/<run-id> --foreground`
- Status: `node <resolved-helper-path> status --root ${TMPDIR:-/tmp}/tunan/brainstorm-visual/<run-id>`
- Stop: `node <resolved-helper-path> stop --root ${TMPDIR:-/tmp}/tunan/brainstorm-visual/<run-id>`
...
Use OS temp by default because visual probes are disposable scratch — never hardcode `/tmp`. Resolve the temp root per OS: `${TMPDIR:-/tmp}` in bash (Git Bash on Windows resolves this), `$env:TEMP` in PowerShell. The `<scratch-root>` below stands for that resolved path:

```text
<scratch-root>/tunan/brainstorm-visual/<run-id>/
  screens/
    001-<decision>.html
  state/
    display-info.json
...
Use `.context/tunan/brainstorm-visual/<run-id>/` only when the user explicitly wants to inspect, preserve, or curate the sketches after the session. The final requirement issue (`tunan:req`) is the durable artifact.
