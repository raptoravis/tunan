...
- When rewriting a body that `new-raw` created, **merge** its `kind` / `priority`
...
- When rewriting a `new-raw` body, the sponsor's `## Background / original
