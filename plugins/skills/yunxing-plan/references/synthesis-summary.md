...
This content is loaded when a synthesis-summary phase fires in yunxing-plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in yunxing-plan (did not route to yunxing-debug, yunxing-work, or universal-planning)
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (yunxing-doc-review, yunxing-work) and human reviewers (PR review).
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (yunxing-doc-review, yunxing-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `/yunxing-brainstorm`), "this is just a fix, no plan needed" (suggest `/yunxing-work`), or "I need to investigate first" (suggest `/yunxing-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `/yunxing-brainstorm` to revise the upstream doc).

In either case: stop yunxing-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
