...
- **The implementing agent** (`yunxing-work` or a human) starts from an informed
...
Not every invocation of `yunxing-plan` should produce a plan document. For
genuinely atomic work, the doc is ceremony — the implementer (whether
`yunxing-work` or a human) can act directly without IDed units, KTDs, or
...
- **Implementation Units** (with stable U-IDs) — the discrete units of work,
  sized so each is independently landable. `yunxing-work` consumes these to
