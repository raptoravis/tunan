...
**Document line.** The `**Document:**` header names the review source. For a tunan artifact, show the label, feature issue number, title, and URL — e.g. `tunan:plan comment on #142 — "User auth" (<url>)` when the source is a plan/solution marker comment, or `tunan:req issue #142 — …` when it is the issue body. For a local markdown file (non-artifact source), show the file path. The URL/path lets the reader open the source the review ran against.
...
**Document:** tunan:plan comment on #142 — "User auth plan" (https://github.com/owner/repo/issues/142)
**Type:** plan
