...
- **Routing option C (top-level Append-to-Open-Questions):** every finding routes through `references/open-questions-defer.md` for Open Questions append to the working file. No document edits apply (beyond the Open Questions section additions themselves). For an issue source, run the SYNC-BACK push once after all appends complete (`gh issue edit <N> --body-file <working-file>`) so the appended entries reach the issue body — routing C does not enter the end-of-walk-through Apply batch, so it carries its own push. After all appends complete (or fail), emit the unified completion report.
