...
The walk-through is operationally read-only with respect to the project except for these permitted writes: the in-memory Apply set / decision list (managed by the orchestrator), the in-doc Open Questions appends to the working file (managed by `references/open-questions-defer.md`), the end-of-walk-through batch edits to the working file (the orchestrator's final Apply pass), and — for an issue source — the SYNC-BACK `gh issue edit` that pushes the working file to the issue body. Persona agents remain strictly read-only. Unlike `code-review`, there is no fixer subagent — the orchestrator owns the document edit directly.
