...
# native PowerShell 5.1 twin cross-model-doc-review.ps1. peer-job-runner.py is
...
#
# Runs ONE doc-review judgment persona through ONE or more DIFFERENT model
...
cleanup_peer_scratch() {
  rm -f "${PROMPT_FILE:-}" "${PEERLOG:-}" "${PEERERR:-}" 2>/dev/null || true
  [ -n "${RAW_OUT:-}" ] && rm -f "$RAW_OUT" 2>/dev/null || true
  if [ -n "${PEER_WORKDIR:-}" ] && [ "${PEER_WORKDIR:-}" != "${RUN_DIR:-}" ]; then rm -rf "$PEER_WORKDIR" 2>/dev/null || true; fi
}
...
# doc-review (unlike code-review) has no pre-existing run-artifact dir, and
# the caller is told to pass a fresh path like /tmp/tunan/doc-review/<run-id>/.
