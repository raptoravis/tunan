...
Environment overrides (defaults in parentheses):
  CE_PEER_JOBS_ROOT         base dir (/tmp/tunan)
...
Windows note: the detach is a re-spawned `--supervise-internal` child in its own
process group (no fork/setsid); process-tree kill uses Get-CimInstance + os.kill
(SIGTERM/SIGKILL both map to TerminateProcess). KNOWN GAP: Windows has no
geteuid, so the owner check above is a no-op there — the predictable %TEMP%
job root is NOT guarded against planted files by ownership on Windows the way it
is on POSIX. The O_EXCL/no-follow/atomic-rename layer still prevents clobbering;
the missing piece is the planted-file owner check (a full fix needs a Windows
ACL read, out of scope for stdlib). Avoid sharing the Windows temp dir with
untrusted users, or set CE_PEER_JOBS_ROOT to a private directory.

Pure stdlib. No third-party dependencies.
"""
# Parity: byte-identical across code-review, doc-review, and pov
# (skills/<skill>/scripts/peer-job-runner.py). Apply any change to ALL THREE in
# the same commit — no automated parity test. Verify: md5sum skills/*/scripts/peer-job-runner.py
import argparse
import glob
import json
import os
...
DEFAULT_ROOT = os.path.join(tempfile.gettempdir(), "tunan")
IS_WINDOWS = os.name == "nt"
_SCRIPT_PATH = os.path.abspath(__file__)  # the Windows supervisor re-spawns this path
# _SIGKILL is absent on Windows Python; os.kill(pid, SIGTERM) there maps
# to TerminateProcess (the only force-kill), so reuse it as the force signal.
_SIGKILL = getattr(signal, "SIGKILL", signal.SIGTERM)
# Bounded wait for the detached supervisor's 2-byte startup ack. If supervise()
# hangs before writing the pid file, cmd_start must error rather than block
# forever — the runner's contract is that every tool call stays short.
ACK_TIMEOUT = float(os.environ.get("CE_PEER_JOBS_ACK_TIMEOUT", "10"))
...
def _pid_running(pid: int) -> bool:
    ...
    if not _pid_alive(pid):
        return False
    if IS_WINDOWS:
        # Windows reaps dead children immediately (no <defunct> zombie state), so
        # kill -0 liveness is sufficient; the ps zombie check is POSIX-only.
        return True
    try:
        out = subprocess.run(
        ...
        # ps unavailable (e.g. no procps on PATH): re-check liveness rather than
        # trusting the earlier _pid_alive guard, so a worker that died between
        # that guard and this ps call is not misclassified as alive (timeout).
...
def _killpg_quiet(pgid: int, sig: int) -> bool:
    if IS_WINDOWS:
        return False  # no process groups on Windows; callers fall through to tree kill
    try:
        os.killpg(pgid, sig)
        return True
...
def _descendants_windows(root_pid: int):
    """Windows tree enumeration via Get-CimInstance (wmic is removed on current
    Windows builds). One call lists every pid/ppid pair; BFS the descendants
    deepest-first so children die before parents can orphan them."""
    try:
        cp = subprocess.run(
...
def _descendants_deepest_first(root_pid: int):
    ...
    if IS_WINDOWS:
        return _descendants_windows(root_pid)
    try:
        out = subprocess.run(
...
def kill_tree(root_pid: int, grace: float) -> bool:
    ...
    while time.monotonic() < deadline:
        if leader_alive and not _pid_alive(root_pid):
            break
        time.sleep(0.1)
    _killpg_quiet(root_pid, _SIGKILL)
    for pid in survivors:
        _kill_quiet(pid, _SIGKILL)
    _kill_quiet(root_pid, _SIGKILL)
    return leader_alive
...
def _reap_worker(proc, conf: dict) -> None:
    ...
    if proc.poll() is None:
        _killpg_quiet(proc.pid, _SIGKILL)
...
def supervise(job_dir: str, argv, result_path, conf: dict, ack_fd: int) -> None:
    ...
    try:
        ...
        try:
            popen_kwargs = dict(
                stdin=devnull, stdout=log_fd, stderr=log_fd, close_fds=True,
            )
            if IS_WINDOWS:
                # Worker leads its own group (CTRL_BREAK signalable); start_new_session
                # is the POSIX analog and is silently ignored on Windows.
                popen_kwargs["creationflags"] = subprocess.CREATE_NEW_PROCESS_GROUP
            else:
                popen_kwargs["start_new_session"] = True  # worker leads its own group: reap = killpg
            proc = subprocess.Popen(argv, **popen_kwargs)
        finally:
            os.close(devnull)
        pid_doc = {
            "supervisor_pid": os.getpid(),
            # pgid is a POSIX concept; None on Windows (no killpg -> tree kill only).
            "supervisor_pgid": (os.getpgid(0) if not IS_WINDOWS else None),
            "worker_pid": proc.pid,
        }
    ...
    if IS_WINDOWS:
        # No process groups: sweep the worker tree deepest-first so children the
        # worker spawned cannot outlive the terminal record.
        _signal_group_or_tree(proc.pid, signal.SIGTERM)
        _signal_group_or_tree(proc.pid, _SIGKILL)
    else:
...
def detach_supervisor(job_dir: str, argv, result_path, conf: dict) -> bool:
    ...
    if IS_WINDOWS:
        return _detach_supervisor_windows(job_dir)
    sys.stdout.flush()
    sys.stderr.flush()
...
def _detach_supervisor_windows(job_dir: str) -> bool:
    """Windows detach: no fork/setsid. Re-spawn this script in a new process
    group to run supervise() via the hidden --supervise-internal entry. The
    supervisor re-reads meta.json for argv/result_path and re-derives conf from
    the inherited environment; it acks by writing the pid file, which the parent
    polls (Windows pipes do not support select on file descriptors, so the
    POSIX pipe-and-select ack is replaced by a pid-file poll)."""
    cmd = [sys.executable, _SCRIPT_PATH, "--supervise-internal", job_dir]
    try:
        subprocess.Popen(
            cmd,
        ...
        write_terminal(job_dir, "failed", f"could not detach supervisor: {exc}", overwrite=False)
        return False
...
def cmd_supervise_internal(job_dir: str) -> int:
    """Hidden entry: the Windows-detached supervisor process runs here. Recovers
    worker_argv/result_path from meta.json (ownership-checked) and conf from the
    environment, then runs supervise(). The pid file supervise() writes is the
    parent's ack (ack_fd=None on Windows -> ack() is a no-op)."""
    try:
        meta = json.loads(read_owned(os.path.join(job_dir, "meta.json"), META_READ_CAP))
    except (Unreadable, OSError, ValueError) as exc:
        write_terminal(job_dir, "failed", f"supervise-internal: cannot read meta.json: {exc}", overwrite=False)
        return 1
    if not isinstance(meta, dict):
        write_terminal(job_dir, "failed", "supervise-internal: meta.json is not an object", overwrite=False)
        return 1
    argv = meta.get("worker_argv")
    result_path = meta.get("result_path")
    if not isinstance(argv, list) or not argv:
        write_terminal(job_dir, "failed", "supervise-internal: meta.json has no worker_argv", overwrite=False)
        return 1
    supervise(job_dir, argv, result_path, cfg(), None)
    return 0
...
def main(argv) -> int:
    # Hidden internal entry: the Windows-detached supervisor re-enters here to
    # run supervise() in its own process (Windows has no fork to inherit state).
    # It recovers argv/result_path from meta.json; acks via the pid file the
    # parent polls. Intercepts before argparse (not a user-facing subcommand).
    if len(argv) == 2 and argv[0] == "--supervise-internal":
        return cmd_supervise_internal(argv[1])
    worker_argv = []
    if "--" in argv:
        split = argv.index("--")
