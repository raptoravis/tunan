...
`yunxing-code-review` is review-only. LFG applies eligible fixes itself, then commits.
...
4. If `git status --short` shows changes, stage only review-driven files, commit `fix(review): apply review findings`, and push before step 5. To push: if an upstream exists, run `git push`. If no upstream exists (common on a fresh feature branch, since step 7's `yunxing-commit-push-pr` has not run yet), resolve a writable remote dynamically: prefer `origin` when present, otherwise use `git remote` and choose the first configured remote. Then run `git push --set-upstream <remote> HEAD`. If no eligible fixes were applied, note explicitly and skip commit.
