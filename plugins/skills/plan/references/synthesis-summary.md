...
This content is loaded when a synthesis-summary phase fires in plan. There are two variants — they share structure but differ in timing and content focus:
...
- AND Phase 0.4 stayed in plan (did not route to debug, work, or universal-planning)
...
When the skill is invoked from an automated workflow such as LFG or any `disable-model-invocation` context, the skill runs in non-interactive mode (no synchronous user). The artifact is read by downstream skills (doc-review, work) and human reviewers (PR review).
...
In either case: stop plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
