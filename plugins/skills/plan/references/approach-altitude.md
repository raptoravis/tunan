...
**Save for later.** Persist the approach-plan so it survives. For knowledge-work deliverables, note in the plan that it is a non-code execution plan — so a later `work` invocation on the saved plan routes to the carve-out (`references/non-code-execution.md` in work), not the code path. Offer to deepen it. Keep the plan **agent-agnostic** (no `work`-specific choreography in the body) so any agent can execute it later.

**Execute now -- code deliverable.** The approach-plan's job is done; continue into the normal `plan` flow (Phase 0.1b onward) to produce the implementation plan, then hand off to `work` for the code. `plan` never writes the code itself.

**Execute now -- non-code deliverable.** This is the knowledge-work path with no `work`-code equivalent. Produce the deliverable directly here, following the approach-plan's intended shape. `plan` itself does not run code in any path.
