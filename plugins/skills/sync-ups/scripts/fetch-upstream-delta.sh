...
# On clone failure prints the single sentinel __SYNCUPS_CLONE_FAILED__.
# Never deletes or writes anything outside the scratch dir.

set -u

LAST_SHA="${1:-}"
SCRATCH="${TMPDIR:-/tmp}/tunan-syncups-upstream"
REPO="https://github.com/everyinc/compound-engineering-plugin.git"
SKILLS_PATH="plugins/compound-engineering/skills/"

rm -rf "$SCRATCH" 2>/dev/null
git clone --quiet "$REPO" "$SCRATCH" 2>/dev/null || { echo "__SYNCUPS_CLONE_FAILED__"; exit 0; }
cd "$SCRATCH" 2>/dev/null || { echo "__SYNCUPS_CLONE_FAILED__"; exit 0; }
...
if [ -n "$LAST_SHA" ] && git cat-file -e "${LAST_SHA}^{commit}" 2>/dev/null; then
  ...
  echo "=== COMMITS ${LAST_SHA}..HEAD ==="
  echo "__SYNCUPS_LAST_SHA_MISSING__"
  echo "=== CHANGED FILES (skills) ==="
  echo "__SYNCUPS_LAST_SHA_MISSING__"
fi
