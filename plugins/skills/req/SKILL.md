...
`$ARGUMENTS` is forwarded verbatim to `brainstorm` (free-text feature description OR a `tunan:req` issue ref — a `#<N>` token or full GitHub issue URL to resume). Do not pre-interpret it here; `brainstorm`'s Phase 0 owns the parsing, GH preflight, resume, scoping, and issue write.
