# tunan-sessions terminology-preservation grader

This grader evaluates whether tunan-sessions findings preserve enough terminology resolution context to make downstream vocabulary capture (tunan-compound Phase 2.4) work. It is NOT a general quality grader for tunan-sessions; the narrow question is "would Phase 2.4 be able to extract qualifying domain terms from these findings?"
...
2. **The findings text** that tunan-sessions returned to the orchestrating agent.
3. **(Optional) The full agent transcript** for the tunan-sessions invocation, if available — useful for distinguishing "tunan-sessions returned this and the agent paraphrased it" from "tunan-sessions returned this verbatim."
...
If Stage 1 fails, tunan-sessions is dropping the most distinctive coined terms — synthesis loss is severe and Stage 2 is moot. Record the failure and stop.
...
- **`preserved` (1.0)** — the finding text discusses the term AND its meaning, role, or the reasoning behind it. Example: "synthesis gate was introduced to prevent tunan-plan from silently proceeding past synthesis without showing the user a Stated/Inferred/Out of scope summary."
...
- "Not relevant" mentions are fine; surfacing the tunan-compound feature PR work as if it answered a CI/CD deployment query is the failure mode.
...
```json
{
  ...
  "stage_1": {
    ...
    "matched_terms_by_tier": {
      "must": ["synthesis gate", "tunan-plan"],
...
| False positive | Eval #4 surfaces the tunan-compound mode:headless work as relevant to CI/CD deployment query | grading.json eval-4 `stage_2.passed: false` |
