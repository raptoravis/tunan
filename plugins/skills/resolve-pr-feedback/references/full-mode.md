...
**Windows / PowerShell** — use the `.ps1` twin (same args and stdout contract; `GH_HOST` is honored by `gh` regardless of shell):
```powershell
$SKILL_DIR = "<absolute path of the directory containing the resolve-pr-feedback SKILL.md>"
$env:GH_HOST = "<derived-host>"   # omit on github.com
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$SKILL_DIR/scripts/get-pr-comments.ps1" PR_NUMBER OWNER/REPO
```
...
Read `references/agents/pr-comment-resolver.md` and spawn a `tunan:pr-comment-resolver` agent for each fix-list item. The fixer is a pure executor: the validity judgment is already done, so it implements and returns — it does not re-judge worthwhileness.
