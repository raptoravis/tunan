...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new tunan-plan / tunan-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- Requirements document → `tunan-plan`
- Plan document → `tunan-work`
...
- Do not modify caller skills (tunan-brainstorm, tunan-plan, or external plugin skills that invoke tunan-doc-review)
