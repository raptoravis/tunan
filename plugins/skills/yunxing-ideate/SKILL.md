...
`yunxing-ideate` precedes `yunxing-brainstorm`.

- `yunxing-ideate` answers: "What are the strongest ideas worth exploring?"
...
3. **Route action into brainstorming** - Ideation identifies promising directions; `yunxing-brainstorm` defines the selected one precisely enough for planning. Do not skip to planning from ideation output.
...
- Never ask about solution direction, constraints, audience, tone, success criteria, or anything that characterizes the subject — those belong to `yunxing-brainstorm`.
...
- Stop as soon as the subject is identifiable or the user has delegated to "Surprise me." More than 3 total questions across 0.2 and 0.4 is a smell that ideation is not the right workflow — consider suggesting `yunxing-brainstorm`.
