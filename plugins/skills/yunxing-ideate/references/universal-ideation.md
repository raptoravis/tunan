...
- **Brainstorm a selected idea** — go deeper on one idea through dialogue. Unlike repo mode, this is not the first step of an implementation chain — there is no `yunxing-plan` → `yunxing-work` after; `yunxing-brainstorm` in universal mode develops the idea further (e.g., expands a name into a brand brief, a plot into an outline, a decision into a weighed framework) and ends there. Persist first per the §6.3 contract in `references/post-ideation-workflow.md`: save the survivors to Proof (the elsewhere-mode default) or to a `yunxing:idea` issue when the user explicitly asks for the issue record, mark the chosen idea as `Explored`, then load `yunxing-brainstorm` with that idea as the seed (pass the `yunxing:idea` issue ref `#<N>` when one was created). On a successful Proof return (`proceeded` or `done_for_now`), continue into the brainstorm handoff per §5.2's caller-aware return rule; on `aborted`, return to this menu without handing off. On persistent Proof failure, apply the §6.5 Proof Failure Ladder before ending so the brainstorm seed is preserved through the issue-save fallback.
- **Save and end** — share the survivors to Proof (the elsewhere-mode default) and end. Create a `yunxing:idea` issue instead only when the user explicitly asks for the issue record. On Proof failure (including after the single orchestrator-side retry), apply the §6.5 Proof Failure Ladder from `references/post-ideation-workflow.md` — surface the issue-save fallback menu (or skip) before ending so the user is not stranded without a recovery path.
