...
Present post-completion options through the platform's blocking question tool (`AskUserQuestion` in Claude Code — call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded; `request_user_input` in Codex; `ask_user` in Gemini/Pi). `AskUserQuestion` caps at 4 options (a hard platform limit — a 5th is rejected by the schema), so fire the tool with the 4 highest-priority destinations — `Run /tunan:code-review` (Recommended), `Create PR`, `Run /tunan:compound`, `Continue` with more experiments — and name the remaining `Done` destination in the question stem so the user can reach it via free-form input (e.g., "…or say 'done' to leave the optimization branch for manual review"). Do **not** fall back to a numbered list to fit all five — option count never justifies dropping the blocking tool. Fall back to a numbered list only when no blocking tool exists or the call errors. Wait for the user's reply; never silently pick or skip. The five options (with execution detail) are:
