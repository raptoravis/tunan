---
name: yunxing-work-beta
description: "[BETA] Execute work with external delegate support. Same as yunxing-work but includes experimental Codex delegation mode for token-conserving code implementation."
...
If it shows an unresolved command string, read `.yunxing/config.local.yaml` from the repo root using the native file-read tool (e.g., Read in Claude Code, read_file in Codex). If the file does not exist, all settings fall through to defaults.
...
7. **Frontend Design Guidance** (if applicable)
   ...
   - Load the `yunxing-frontend-design` skill before implementing
