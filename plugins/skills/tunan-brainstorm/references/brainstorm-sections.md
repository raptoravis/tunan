...
- **The planning agent** (`tunan-plan` or a human) produces an implementation
...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`tunan-plan`, the commit message, `docs/solutions/`)
...
decision flows to `tunan-plan` (or directly to commit message, or to
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("tunan-doc-review can act on this
...
- **`topic`** — kebab-case slug identifying the brainstorm subject (e.g.,
  ...
  alongside `date` and as the resume-detection key when `tunan-brainstorm`'s
...
downstream consumers (`tunan-plan`, `tunan-doc-review`) reference via the plan's
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to tunan-plan, no italic provenance lines. Engineering process
