...
consumers that read HTML today (`tunan-work`, human readers) do so directly;
the agent-consumability rules below make that work. `tunan-doc-review` is
_not_ currently an HTML consumer — its mutation mechanics are markdown-only,
so the tunan-plan handoff gates the 5.3.8 doc-review pass to `OUTPUT_FORMAT=md`
...
- **Editable status renders as `<span class="status">{value}</span>`.**
  Downstream tooling (`tunan-work` shipping flip, future HTML-aware
...
- **Source / composition signal.** A visible footer at the bottom of
  ...
  `<footer class="composition-signal">Composed 2026-05-17T14:23Z by tunan-plan from <code>docs/brainstorms/...-requirements.md</code></footer>`.
...
Downstream agents that read HTML today (`tunan-work`, future consumers) read
the HTML file as text linearly, not via DOM extraction. `tunan-doc-review` is
