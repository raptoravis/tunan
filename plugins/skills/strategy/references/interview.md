...
Once sections 1-5 are captured (and any optional sections the user engaged with), read `strategy-template.md` and fill in the intent block. Present the full draft in chat before writing. Offer one edit round. Then merge it into the `tunan:project` issue's intent sections (per `SKILL.md` Phase 1/2), preserving the roadmap.
