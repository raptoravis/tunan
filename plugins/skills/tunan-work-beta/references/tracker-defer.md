...
This reference covers how residual actionable findings are filed in the project's tracker. Loaded by caller workflows (for example `tunan-work` Residual Work Gate, or `lfg` residual handling) — not by `tunan-code-review`, which stops after the report.
...
Used by `tunan-work` Residual Work Gate and similar caller flows when the user chooses to file tickets. All user-facing prompts fire:
...
- **Length cap:** when the composed body would exceed a tracker's body length limit, truncate with `... (continued in tunan-code-review run artifact: ${TMPDIR:-/tmp}/tunan/tunan-code-review/<run-id>/)` and include the finding_id in both the truncated body and the metadata block so the artifact is discoverable.
