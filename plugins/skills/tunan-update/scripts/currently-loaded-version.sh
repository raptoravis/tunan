...
# marketplace cache layout `~/.claude/plugins/cache/<marketplace>/tunan/<version>/skills/tunan-update`,
...
# Match `.../plugins/cache/*/tunan/<version>/skills/tunan-update[/]?`
# Capture group 1 is the version segment.
version=$(printf '%s\n' "$skill_dir" | sed -nE 's|.*/plugins/cache/[^/]+/tunan/([^/]+)/skills/tunan-update/?$|\1|p')
