...
- `agent-browser` installed. Check:
  ...
  If not installed, run the `yunxing-setup` skill to install dependencies, then resume. Do not continue without it.

## Reusing Compound-Engineering Skills

`yunxing-dogfood-beta` is an orchestrator. Prefer delegating to existing CE skills over re-deriving their behavior:

| When | Skill | Why |
|------|-------|-----|
| Phase 0 isolation | `yunxing-worktree` | Run the dogfood in an isolated worktree so the main checkout stays clean. |
| agent-browser missing | `yunxing-setup` | Installs `agent-browser` and other deps. |
| A failure's root cause is non-obvious | `yunxing-debug` | Systematic root-cause analysis instead of guess-and-check. |
| Committing each fix | `yunxing-commit` | Consistent, well-scoped commit messages. |
| A bug reveals a reusable lesson | `yunxing-compound` | Capture the learning so the team compounds knowledge. |

Reuse `yunxing-test-browser`'s mechanics for port detection and dev-server startup (see Phase 3) rather than reinventing them.
...
Determine the port (priority: explicit `--port` > `AGENTS.md`/`CLAUDE.md` > `package.json` dev script > `.env*` `PORT=` > default `3000`). If a server is already listening, reuse it; otherwise start the project's dev command in the background and wait for the port to come up. This is the same mechanism `yunxing-test-browser` uses — follow its Phase 5–6 logic.
...
1. Investigate the root cause. If it's non-obvious, use `yunxing-debug`.
...
4. Commit the fix with a clear message (use `yunxing-commit`). One logical fix per commit.
5. Re-run the failing scenario in the browser to confirm it now passes; then continue the matrix.
6. If the bug carried a reusable lesson, capture it with `yunxing-compound`.
...
8. **Learnings** — reusable lessons worth carrying forward (feed substantial ones to `yunxing-compound`).
