...
3. **Read the sync baseline.** Read `.tunan/upstream-sync.json` for
   `last_synced_sha`. If the file is missing, treat the merge as a full audit
   (no baseline diff) and tell the user the baseline will be created at the end.
...
1. **Update the sync baseline.** Rewrite `.tunan/upstream-sync.json` with
   `last_synced_sha` = `UPSTREAM_HEAD` and `last_synced_date` = today.
