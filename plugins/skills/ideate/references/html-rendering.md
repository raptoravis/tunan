...
`brainstorm` writes (now under `docs/plans/`), not an implementation-ready
plan (`plan`'s enriched output) — and only to UI-shaped requirements — a
...
Downstream agents that read HTML today (`work`, a skill re-reading its
...
script-style parse. `doc-review` is not a current HTML consumer (see
opening note).
