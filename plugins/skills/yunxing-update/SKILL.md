...
name: yunxing-update
description: |
  Check if the yunxing plugin is up to date and recommend the
  ...
  stale yunxing plugin version. This skill only works in Claude
...
Verify the installed yunxing plugin version matches the upstream
...
`~/.claude/plugins/cache/<marketplace>/yunxing/<version>/skills/yunxing-update`.
...
> Your marketplace install (if any) is unaffected — run `/yunxing-update` in a
...
> "yunxing **v{version}** is installed and up to date."

**Out of date** — `currently_loaded != upstream`:

> "yunxing is on **v{currently_loaded}** but **v{upstream}** is available.
>
> Update with:
>
> ```
> claude plugin update yunxing@{marketplace_name}
> ```
>
...
multiple marketplace names (for example, `yunxing-plugin` for
