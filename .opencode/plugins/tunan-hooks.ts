...
let _bootstrapCache: string | undefined = undefined;
...
function getBootstrapContent(): string {
  ...
  if (fs.existsSync(instructionsFile)) {
    try {
      ...
      console.error('[tunan] failed to read instructions file:', (e as Error).message);
    }
  }
...
const createTunanHooksPlugin = async () => ({
  ...
  'experimental.chat.messages.transform': async (_input: any, output: any) => {
    try {
      ...
      console.error('[tunan] messages.transform error:', (e as Error).message, (e as Error).stack);
    }
  },
  ...
  'session.created': async (_ctx: any) => {
